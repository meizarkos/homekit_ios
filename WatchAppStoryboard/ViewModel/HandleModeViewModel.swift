...
class HandleModeViewModel: ObservableObject {
    ...
    private func autoSelectAccessories() {
        ...
        // Température
        if selectedTemp == nil {
            selectedTemp = home.accessories.first
            if let accessory = selectedTemp {
                print("Auto-selected temp accessory: \(accessory.name)")
            }
        }
    }
    ...
    func callSoundHomekit() {
        ...
        homeKitManager.setSpeakerVolume(accessory, volume: sound)
    }
    ...
    func callTempHomekit() {
        ...
        guard let accessory = selectedTemp else {
            print("No selected temp accessory")
            return
        }
        
        homeKitManager.setTargetTemperature(accessory, temperature: temperature)
    }
